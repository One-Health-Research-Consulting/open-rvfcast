...
```
dataset <- arrow::open_dataset(arrow::s3_bucket("open-rvfcast/data/explanatory_variables/", anonymous = TRUE))
dataset$schema
```
...
Many of the computational steps in the first module can be time consuming and either depend on or produce large files. In order to speed up the pipeline, intermediate files can be stored on the cloud for portability. We currently use an AWS [S3 bucket](https://aws.amazon.com/s3/) for this purpose. The pipeline will still run without access to cloud storage, but users can add their own AWS access keys and bucket ID to the `.env` file to enable cloud storage. 
...
```
tar_make(africa_full_rvf_model_data, script = "data_acquisition_targets.R")
```
