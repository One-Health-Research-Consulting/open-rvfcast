...
, tar_target(rvf_response_directory, create_data_directory(directory_path = "data/rvf_response"))
  ...
  ## Identify space-time linked outbreak chains and flag country-level index cases.
   ## The first appearance of a chain within a given country gets flagged in order
   ## to identify "economically important index cases".
   ## Within-country space and time thresholds are larger than cross-border
   ## thresholds given the theory of faster within-country response once cases
   ## have already been detected in that country
, tar_target(rvf_outbreaks_indexed,
    identify_index_outbreaks(
      wahis_outbreaks                    = rvf_outbreaks
    , space_threshold_km                 = 250
    , time_threshold_days                = 60
    , within_country_space_threshold_km  = 2000
    , within_country_time_threshold_days = 90
    , country_polygons                   = continent_polygon))
...
, tar_target(rvf_response,
    get_rvf_response(
      wahis_outbreaks       = rvf_outbreaks_indexed
  ...
  ## Build a neighbor observed-outbreak history term: per hex, per model date, case-weighted
   ...
   ## NOTE: May or may not be used in modeling (can be stripped out in model_framework_targets.R)
    ## given potential issues generated by reporting lags for forecasting
