...
fold_data <- function(
  ...
  coords.sorted <- sf_districts |>
    dplyr::select(region, cluster, area_km2) |>
    mutate(area_km2 = log(area_km2)) |>
    rename(!!district_id_col := region) |>
    as.data.frame() |>
  ...
  for (i in seq_len(nrow(outer_folds))) {
    ...
    tdat <- data |>
    filter(index %in% (
        outer_folds[i, ] |> pull(train_data) |> unlist()
        )) |>
      left_join(coords.sorted, by = c("shapeName")) |>
...
build_all_inner_folds <- function(folded_data) {
 ...
 all_inner_folds <- purrr::map(seq_len(nrow(folded_data)), function(fold) {
