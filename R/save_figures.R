...
save_fig_pieces <- function(input, outpath, idinfo, plotname, overwrite) {
  ...
  if (plotname != "map_split") {
   for (i in seq_along(unifold)) {
    ...
    try({ ggsave(out_file, plot = gg.t, width = 8, height = 7)}, silent = TRUE)
    ...
    te.t     <- te %>% filter(aggregation == uniag[i])
      ...
      for (p in 1:nrow(te.t)) {
        ...
        if (!file.exists(out_file) | overwrite) {
          ...
          for (p in 1:nrow(te.t)) { print(te.t[p, ]$map_split[[1]]) }
          dev.off()
    ...
    for (i in seq_along(uniag)) {
      ...
      if (!file.exists(out_file) | overwrite) {
        try({ ggsave(out_file, plot = te.t[p, ]$map_split[[1]], width = 8, height = 7)}, silent = TRUE)
      }
