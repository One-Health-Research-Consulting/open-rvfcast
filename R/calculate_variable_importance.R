...
compare_vi <- function(variable_importance) {
  ...
  d.temp <- variable_importance$top_variables_xgb |>
    bind_rows() |>
    group_by(Feature) |>
    ...
    vi.gg <- ggplot(d.temp, aes(Gain, Feature)) +
  ...
  d.temp <- variable_importance$pdp_df |> bind_rows()

    pdp.gg <- ggplot(d.temp, aes(value, yhat)) +
