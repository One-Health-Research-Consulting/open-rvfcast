...
make_area_clusters        <- function(
  ...
  regions_eq.r <- regions_eq.r |> mutate(regions = list(rep(character(1), nrow(.data))))
  ...
  if (file.exists(path_to_clustered_regions) && !overwrite) {
    ...
    ## Tidy region names
    for (z in seq_len(nrow(clusts.t))) {
  ...
  for (i in seq_len(nrow(clusts.t))) {
  ...
  while (length(remaining) > 0L) {
    ...
    for (k in seq_len(K)) {
      ...
      if (growth_option == "balanced") {
        ...
        j     <- cand[sample(seq_along(score), 1)]
      } else {
