...
build_app_components <- function(predictions, outpath) {
  ...
  no_agg.map       <- purrr::map(files_tibble.map$no_agg      , .f = function(i) { qread(i) }) %>% bind_rows()
  double_agg.map   <- purrr::map(files_tibble.map$double_agg  , .f = function(i) { qread(i) }) %>% bind_rows()
  temporal_agg.map <- purrr::map(files_tibble.map$temporal_agg, .f = function(i) { qread(i) }) %>% bind_rows()
  spatial_agg.map  <- purrr::map(files_tibble.map$spatial_agg , .f = function(i) { qread(i) }) %>% bind_rows()
  
  no_agg.cal       <- purrr::map(files_tibble.cal$no_agg      , .f = function(i) { qread(i) }) %>% bind_rows()
  double_agg.cal   <- purrr::map(files_tibble.cal$double_agg  , .f = function(i) { qread(i) }) %>% bind_rows()
  temporal_agg.cal <- purrr::map(files_tibble.cal$temporal_agg, .f = function(i) { qread(i) }) %>% bind_rows()
  spatial_agg.cal  <- purrr::map(files_tibble.cal$spatial_agg , .f = function(i) { qread(i) }) %>% bind_rows()
  ...
  return(paste(outpath, "/data_for_app.qs", sep = ""))
