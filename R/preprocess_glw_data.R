...
preprocess_glw_data<- function(glw_directory_dataset, glw_directory_raw, glw_downloaded, continent_raster_template) {
  ...
  transformed_raster_cat <- transform_raster(raw_raster = terra::rast(paste0(glw_downloaded, "/url_cattle.tif")),
                                             template = terra::rast(continent_raster_template))
  transformed_raster_sh <- transform_raster(raw_raster = terra::rast(paste0(glw_downloaded, "/url_sheep.tif")),
                                            template = terra::rast(continent_raster_template))
  transformed_raster_go <- transform_raster(raw_raster = terra::rast(paste0(glw_downloaded, "/url_goats.tif")),
                                            template = terra::rast(continent_raster_template))
